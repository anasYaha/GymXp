...
import { authStore, setAuthStore } from "../features/auth/auth.store";
...
import { TrainingProvider } from "../store/training-store";
...
export const MobileAppRoot = () => {
  ...
  const loadMemberData = async () => {
    const [nextSummary, nextSessions] = await Promise.all([
      getDashboardSummary(JSON.parse(await import("@react-native-async-storage/async-storage").then(m => m.default.getItem("@gymxp_training_state")) || "{}")?.days?.find((d: any) => d.day === ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"][new Date().getDay()])?.type || undefined),
      listAvailableSessions()
    ]);

    const refreshedUser = authStore.user
      ? {
          ...authStore.user,
          level: nextSummary.user.level,
          totalXp: nextSummary.user.totalXp,
          totalSessions: nextSummary.user.totalSessions,
          currentStreak: nextSummary.user.currentStreak,
          levelXpProgress: nextSummary.user.levelXpProgress,
          levelDaysProgress: nextSummary.user.levelDaysProgress
        }
      : null;
  ...
  if (booting) {
    return (
      ...
      >
        ...
        <Text style={{ color: "#AAAAAA", fontSize: 14 }}>Loading GymXP...</Text>
      </View>
    );
  }
  ...
  if (!summary) {
    return (
      ...
      >
        <ActivityIndicator size="large" color="#FF6B00" />
        <Text style={{ color: "#AAAAAA", fontSize: 14 }}>Loading dashboard...</Text>
      </View>
    );
  }
